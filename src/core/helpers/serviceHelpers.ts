...
export async function onlineOnly<T>(
        onlineFn: () => Promise<T>,
        // @ts-ignore // pour dire que la prochaine ligne tu me l'analyse pas
...
function handleNoDataFound(e: any) { // uniquement pour le online
...
export function isOnlineNotFoundError(e: any) {
    return e instanceof Error && (e.message === "Failled to fetch" || e.message === "Network Error" || e.message === "Request timeout");
}
